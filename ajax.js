$('#addRoom').submit(function () {
    ...
    var hike_route =$('#hike_route').val();
    var date_taken =$('#date_taken').val();
    var coordinator =$('#coordinator').val();
    var hour_taken =$('#hour_taken').val();
    var difficult_level =$('#difficult_level').val();
    var km =$('#km').val();
    $.ajax({
        type: 'post',
        url: 'ajax.php',
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.done == true){
                ...
                $('.emp-response').html('<div class="alert bg-success alert-dismissable" role="alert"><em class="fa fa-lg fa-warning">&nbsp;</em>Data Successfully Added</div>');
            }else{
