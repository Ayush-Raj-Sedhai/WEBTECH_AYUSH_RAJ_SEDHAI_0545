...
if (isset($_POST['add_room'])) {
    ...
    if ($room_no != '') {
        ...
        if (mysqli_num_rows(mysqli_query($connection, $sql)) >= 1) {
            $response['done'] = false;
            $response['data'] = "ID Already Exist";
        } else {
         $query = "INSERT INTO room (room_type_id,room_no,organizer,date_taken,hiking_route,coordinator,hour_taken,difficult_level,km) VALUES ('$room_type_id','$room_no', '$organizer', '$date_taken','$hiking_route','$coordinator','$hour_taken','$difficult_level','$km')";
            $result = mysqli_query($connection, $query);
...
if (isset($_POST['edit_room'])) {
    ...
    if ($room_no != '') {
        ...
        if ($result) {
            $response['done'] = true;
            $response['data'] = 'Successfully Edit Data';
        } else {
            $response['done'] = false;
            $response['data'] = "DataBase Error";
        }
        ...
        $response['done'] = false;
        $response['data'] = "Please Fill Form";
    }
...
if (isset($_POST['room_type'])) {
    ...
    $result = mysqli_query($connection, $sql);
    if ($result) {
        echo "<option selected disabled>Select Hike Type</option>";
...
if (isset($_POST['booking'])) {
    ...
    if ($customer_result) {
        ...
        $response['done'] = false;
        $response['data'] = "DataBase Error Add";
    }
    ...
    if ($booking_id != '') {
        ...
        if ($result) {
            ...
            if ($updateResult) {
                ...
                $response['done'] = false;
                $response['data'] = "Problem in Update";
            }
        } else {
            $response['done'] = false;
...
if (isset($_POST['check_out_room'])) {
    ...
    if ($booking_id != '') {
        ...
        $response['done'] = false;
        $response['data'] = "Error With Adding";
    }
    echo json_encode($response);
}
